...
function getBoiPointSum(matches, heropoints) {
    ...
    for (i = 0; i < matches.length; i++) {
        var radiantWin = Boolean(matches[i].radiant_win);
        var isRadiant = Boolean(matches[i].player_slot < 128);
        var win;
        ...
        let matchPoints = Math.round((heropoints[matches[i].hero_id] + win)*10)/10;
